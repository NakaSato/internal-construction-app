...
import 'recurring_event_settings.dart';
...
class CalendarEventDialog extends StatefulWidget {
  ...
  const CalendarEventDialog({super.key, this.event, this.selectedDate, this.isEditing = false});
...
class _CalendarEventDialogState extends State<CalendarEventDialog> {
  ...
  void _initializeFields() {
    if (widget.event != null) {
      ...
      _attendeesController = TextEditingController(text: widget.event!.attendees?.join(', ') ?? '');
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildAllDaySwitch(),
                      const SizedBox(height: 16),
                      if (!_isAllDay) ...[_buildTimeFields(), const SizedBox(height: 16)],
  ...
  Widget _buildHeader() {
    return Row(
      children: [
        Icon(widget.isEditing ? Icons.edit : Icons.add, color: Theme.of(context).primaryColor),
      ...
      children: [
        ...
        if (_endDateTime.isBefore(_startDateTime))
          const Padding(
            ...
            child: Text('End time must be after start time', style: TextStyle(color: Colors.red, fontSize: 12)),
          ),
      ],
    );
  }
  ...
  Widget _buildColorPicker() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Event Color (Optional)', style: TextStyle(fontWeight: FontWeight.w500)),
        const SizedBox(height: 8),
        Wrap(
          spacing: 8,
          runSpacing: 8,
  ...
  Widget _buildActions() {
    return Row(
      ...
      children: [
        ...
        const SizedBox(width: 8),
        ElevatedButton(onPressed: _saveEvent, child: Text(widget.isEditing ? 'Update' : 'Create')),
      ],
    );
  }
  ...
  Future<void> _selectDateTime(DateTime currentDateTime, Function(DateTime) onChanged) async {
    ...
    if (date != null && mounted) {
      final time = await showTimePicker(context: context, initialTime: TimeOfDay.fromDateTime(currentDateTime));
  ...
  void _saveEvent() {
    ...
    if (!_isAllDay && _endDateTime.isBefore(_startDateTime)) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('End time must be after start time'), backgroundColor: Colors.red));
      return;
    }
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                context.read<CalendarManagementBloc>().add(CalendarEventDeleted(widget.event!.id));
