...
import '../../../../core/widgets/table_calendar.dart';
...
class CalendarManagementScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => GetIt.instance<CalendarManagementBloc>()..add(const CalendarEventsRequested()),
...
class _CalendarManagementViewState extends State<CalendarManagementView> with SingleTickerProviderStateMixin {
        ...
        children: [
          ...
          BlocBuilder<CalendarManagementBloc, CalendarManagementState>(
            builder: (context, state) {
              ...
              return CalendarAgendaView(events: events, onEventTap: (event) => _showEventDetails(context, event));
            },
          ),
        ],
      ),
              ...
              CustomTableCalendar(
                ...
                onDaySelected: (selectedDay, focusedDay) {
                  ...
                  context.read<CalendarManagementBloc>().add(
                    CalendarEventsRequested(startDate: selectedDay, endDate: selectedDay.add(const Duration(days: 1))),
                  );
                },
                onPageChanged: (focusedDay) {
                  ...
                  context.read<CalendarManagementBloc>().add(
                    CalendarEventsRequested(startDate: monthStart, endDate: monthEnd),
                  );
                },
              ),
              SizedBox(
                height: 300, // Fixed height for events list
                child: _EventsList(onEventTap: (event) => _showEventDetails(context, event)),
              ),
            ],
          ),
        );
      ...
      builder: (context, state) {
        return Column(
          children: [
            ...
            Container(height: 120, padding: const EdgeInsets.symmetric(vertical: 8), child: _buildWeekDaySelector()),

            // Events for selected date within this week
            Expanded(child: _EventsList(onEventTap: (event) => _showEventDetails(context, event))),
          ],
        );
      },
    );
  ...
  Widget _buildWeekHeader() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: theme.colorScheme.shadow.withOpacity(0.05), blurRadius: 8, offset: const Offset(0, 2)),
        ],
      ),
      child: Row(
        ...
        children: [
            ...
            child: IconButton(
              ...
              onPressed: () {
                final newSelectedDay =
                    _selectedDay?.subtract(const Duration(days: 7)) ?? DateTime.now().subtract(const Duration(days: 7));
                setState(() {
          ...
          Container(
            ...
            child: IconButton(
              ...
              onPressed: () {
                final newSelectedDay =
                    _selectedDay?.add(const Duration(days: 7)) ?? DateTime.now().add(const Duration(days: 7));
                setState(() {
                  _selectedDay = newSelectedDay;
                  _focusedDay = newSelectedDay;
                });
      ...
      itemBuilder: (context, index) {
        ...
        return GestureDetector(
          onTap: () {
            ...
            // Load events for selected day
            context.read<CalendarManagementBloc>().add(
              CalendarEventsRequested(startDate: day, endDate: day.add(const Duration(days: 1))),
            );
          },
          child: Container(
      ...
      builder: (context, state) {
        ...
        if (state is CalendarEventsLoaded) {
          dayEvents = state.events.where((event) {
            ...
            final selectedDate = DateTime(dateToShow.year, dateToShow.month, dateToShow.day);
          ...
          Container(
            ...
            child: IconButton(
              ...
              onPressed: () {
                ...
                context.read<CalendarManagementBloc>().add(
                  CalendarEventsRequested(startDate: newDay, endDate: newDay.add(const Duration(days: 1))),
                );
              },
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (event.location != null && event.location.isNotEmpty)
                    Row(
                      children: [
                        Icon(Icons.location_on, size: 12, color: Theme.of(context).colorScheme.onPrimary),
      ...
      builder: (dialogContext) => BlocProvider.value(
        ...
        child: FindTimeWidget(
          ...
          onTimeSlotSelected: (startTime, endTime) {
            ...
            _showCreateEventDialog(context, startTime: startTime, endTime: endTime);
          },
        ),
      ),
    );
  ...
  void _showFilterDialog(BuildContext context) {
    showDialog(
      ...
      builder: (dialogContext) =>
          BlocProvider.value(value: context.read<CalendarManagementBloc>(), child: const CalendarFilterWidget()),
    );
  }
  ...
  void _showSearchDialog(BuildContext context) {
    showDialog(
      context: context,
      builder: (dialogContext) =>
          BlocProvider.value(value: context.read<CalendarManagementBloc>(), child: const CalendarSearchWidget()),
    );
  }

  void _showCreateEventDialog(BuildContext context, {DateTime? startTime, DateTime? endTime}) {
    showDialog(
      context: context,
      builder: (dialogContext) => BlocProvider.value(
        value: context.read<CalendarManagementBloc>(),
        child: CalendarEventDialog(selectedDate: startTime ?? _selectedDay ?? DateTime.now()),
      ),
    );
  }
    ...
    return BlocBuilder<CalendarManagementBloc, CalendarManagementState>(
      builder: (context, state) {
        if (state is CalendarManagementInitial) {
          ...
          return CalendarEventListWidget(events: state.events, onEventTap: onEventTap);
        } else if (state is CalendarSearchResultsLoaded) {
          return Column(
            children: [
              Padding(
              ...
              Expanded(
                child: CalendarEventListWidget(events: state.results, onEventTap: onEventTap),
              ),
            ],
          );
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                Text('Error loading events', style: Theme.of(context).textTheme.headlineSmall),
                const SizedBox(height: 8),
                Text(state.message, style: Theme.of(context).textTheme.bodyMedium, textAlign: TextAlign.center),
                const SizedBox(height: 16),
                ElevatedButton(
                  onPressed: () => context.read<CalendarManagementBloc>().add(const CalendarEventsRequested()),
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                if (state.hasConflicts && state.conflictingEvents.isNotEmpty) ...[
                  const SizedBox(height: 16),
                  Text('${state.conflictingEvents.length} conflicting events found'),
                ],
              ],
            ),
          );
