...
void configureDailyReportsDependencies() {
  ...
  // Note: Dio is now registered by Injectable system
  ...
  // Register repository - disabled in favor of Injectable system
  // if (!getIt.isRegistered<DailyReportRepository>()) {
  //   getIt.registerLazySingleton<DailyReportRepository>(() => MockDailyReportRepository());
  // }

  // Register use cases
  if (!getIt.isRegistered<GetDailyReports>()) {
    getIt.registerLazySingleton<GetDailyReports>(() => GetDailyReports(getIt<DailyReportRepository>()));
  }

  if (!getIt.isRegistered<GetDailyReportById>()) {
    getIt.registerLazySingleton<GetDailyReportById>(() => GetDailyReportById(getIt<DailyReportRepository>()));
  }

  if (!getIt.isRegistered<CreateDailyReport>()) {
    getIt.registerLazySingleton<CreateDailyReport>(() => CreateDailyReport(getIt<DailyReportRepository>()));
  }

  if (!getIt.isRegistered<UpdateDailyReport>()) {
    getIt.registerLazySingleton<UpdateDailyReport>(() => UpdateDailyReport(getIt<DailyReportRepository>()));
  }

  if (!getIt.isRegistered<DeleteDailyReport>()) {
    getIt.registerLazySingleton<DeleteDailyReport>(() => DeleteDailyReport(getIt<DailyReportRepository>()));
  }

  if (!getIt.isRegistered<UploadDailyReportImage>()) {
    getIt.registerLazySingleton<UploadDailyReportImage>(() => UploadDailyReportImage(getIt<DailyReportRepository>()));
  }
